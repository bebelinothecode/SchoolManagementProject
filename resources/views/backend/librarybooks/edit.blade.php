...
@section('content')
    <div class="roles">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h2 class="text-gray-700 uppercase font-bold">Edit book</h2>
            </div>
        ...
        <div class="table w-full mt-8 bg-white rounded">
            <form action="{{ route('updatebook',$book->id) }}" method="POST" class="w-full max-w-xl px-6 py-12" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="md:flex md:items-center mb-6">
                    ...
                    <div class="md:w-2/3">
                        <input name="name_of_book" class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" type="text" value="{{ $book->name_of_book }}">
                        @error('name')
                ...
                <div class="md:flex md:items-center mb-6">
                    ...
                    <div class="md:w-2/3">
                        <input name="date_of_borrow" 
                               class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" 
                               type="date" 
                               value="{{ old('date_of_borrow', $book->date_of_borrow ?? '') }}">
                        @error('date_of_borrow')
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </div>
                </div>
                <div class="md:flex md:items-center mb-6">
                    ...
                    <div class="md:w-2/3">
                        <input name="due_date" 
                               class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" 
                               type="date" 
                               value="{{ old('due_date', $book->due_date ?? '') }}">
                        @error('date_of_borrow')
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="md:flex md:items-center mb-6">
                    ...
                    <div class="md:w-2/3">
                        <input name="date_returned" class="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-blue-500" type="date" value="{{ old('date_returned', $book->date_returned ?? '') }}">
                        @error('roll_number')
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </div>
                        ...
                        <select name="student_id" 
                            ...
                            @foreach ($students as $student)
                                <option value="{{ $student->id }}" 
                                        {{ old('borrower_id') == $student->id ? 'selected' : '' }}>
                    ...
                    <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative" role="alert">
                        ...
                        <span class="absolute top-0 bottom-0 right-0 px-4 py-3">
                            <svg class="fill-current h-6 w-6 text-green-500" role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                <title>Close</title>
                                <path d="M14.348 14.849a1 1 0 01-1.414 0L10 11.414l-2.934 3.435a1 1 0 01-1.514-1.32l3-3.5a1 1 0 011.414 0l3 3.5a1 1 0 010 1.32z"/>
                            </svg>
                        </span>
                    </div>
                @endif
